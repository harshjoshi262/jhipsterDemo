...
gulp.task('less', function () {
    return es.merge(
        ...
        .pipe(plumber({errorHandler: handleErrors}))
        .pipe(expect(config.lessSrc))
        .pipe(changed(config.cssDir, {extension: '.css'}))
        .pipe(less({includePaths:config.bower}).on('error', less.logError))
...
gulp.task('wiredep:app', function () {
    ...
    return es.merge(stream, gulp.src(config.lessSrc)
        .pipe(plumber({errorHandler: handleErrors}))
        .pipe(wiredep({
...
gulp.task('install', function () {
    runSequence(['wiredep', 'ngconstant:dev'], 'less', 'languages', 'inject');
});
